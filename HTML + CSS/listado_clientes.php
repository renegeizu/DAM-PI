	...
	?>
     <div class="container">
		...
		<?php
			...
			if($limite==1){
				$numero=$limite*5;
				$query="SELECT id, nombre, empresa FROM clientes ORDER BY ".$_GET['order']." LIMIT ".$numero;
			}else{
				$numero=($limite-1)*5;
				$query="SELECT id, nombre, empresa FROM clientes ORDER BY ".$_GET['order']." LIMIT ".$numero.", 5";
			}
			$pagina_completa="SELECT id, nombre, empresa FROM clientes ORDER BY ".$_GET['order'];
			...
			if($_SESSION['Puesto']=='Jefe'){
				echo '<table class="table table-striped table-hover">'.'<tr><th>Nombre</th>'.
					'<th>Empresa</th><th>Detalles</th><th>Edicion</th><th>Borrado</th></tr>';
			}else{
				echo '<table class="table table-striped table-hover">'.'<tr><th>Nombre</th>'.
					'<th>Empresa</th><th>Detalles</th></tr>';
			}
			while ($row=mysqli_fetch_array($resultado)) {
				if($_SESSION['Puesto']=='Jefe'){
					echo '<tr><td>'.$row['nombre'].'</td>';
					echo '<td>'.$row['empresa'].'</td>';
					echo '<td><a href="'."detalles_clientes.php?id=".$row['id'].
						'"><img src="imagenes/detalles.png"/></a></td>';
					echo '<td><a href="'."editar_clientes.php?id=".$row['id'].
					'"><img src="imagenes/edicion.png"/></a></td>';
					echo '<td><a href="'."borrar_cliente.php?id=".$row['id'].
